...
async def connect_and_run(com: SocketCom, write_loop_func: Callable[[], Coroutine]):
    async def run_loop_and_terminate_group():
        await write_loop_func()
        raise TerminateReadLoop()

    print_debug_log(
        "Starting in concurrence communication writing loop (S) and communication reading loop (R)"
    )
    try:
        ...
        print_debug_log(
            "Communication writing loop and communication reading loop finished!"
        )


async def mock_write_communication(communicator: SocketCom):
    async def try_or_fail_sending(msg: str, prefix: str):
        try:
            ...
            print_debug_log(f"Timeout in awaiting {prefix}", in_strategy_loop=True)
...
async def mock_communication(communicator: SocketCom):
...
def start():
    ...
    communicator = SocketCom[Literal["G", "C", "UnavoidableTimeout"]](
        {"G", "C", "UnavoidableTimeout"}, config, is_client=True
    )
